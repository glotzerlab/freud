...
class TestLocalBondProjection(unittest.TestCase):
    def test_getNP(self):
        ...
        ang = freud.order.LocalBondProjection(rmax, num_neigh)
    ...
    def test_compute(self):
        ...
        # For the first particle, the bond is [1,0,0] and the orientation is [1,0,0,0]
        # So the local bond is [1,0,0]
        # The projection onto [0,0,1] is cos(pi/2)=0
        npt.assert_almost_equal(ang.getProjections()[0], 0, 6)
        npt.assert_almost_equal(ang.getNormedProjections()[0], 0, 6)
        # For the second particle, the bond is [-1,0,0] and the orientation is rotated about the y-axis by pi/2
        # So the local bond is [0,0,-1]
        ...
        # For the third particle, the bond is [0,0,-1.5] and the orientation is rotated about the z-axis by pi/2
        # So the local bond is [0,0,-1.5]
        ...
        # Specify that rotations about y by +/-pi/2 and rotations about x by pi result in equivalent particle shapes
        qs = [[1,0,0,0],[np.cos(np.pi/4),0,np.sin(np.pi/4),0],[np.cos(np.pi/2),np.sin(np.pi/2),0,0]]
