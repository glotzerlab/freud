...
class TestBondOrder(unittest.TestCase):
    def test_bond_order(self):
        ...
        # the desired results.
        bo.accumulate(box, positions, quats, mode='lbod')
        ...
        # same.
        bo.compute(box, positions, quats, mode='obcd')
        self.assertTrue(np.allclose(bo.bond_order, op_value))
        ...
        bo.compute(box, positions, random_quats)
        self.assertTrue(np.allclose(bo.bond_order, op_value))

        # Ensure that obcd looks random for the randomized orientations.
        bo.compute(box, positions, random_quats, mode='obcd')
        ...
        # same.
        bo.reset()
        bo.accumulate(box, positions, quats, mode='oocd')
        ...
        bo.compute(box, positions, random_quats, mode='oocd')
