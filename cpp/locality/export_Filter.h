...
void compute(std::shared_ptr<Filter> filter, std::shared_ptr<NeighborQuery> nq,
        nb_array<float, nb::shape<-1, 3>> query_points, std::shared_ptr<NeighborList> nlist, const QueryArgs& qargs)
{
...
{
    nb::class_<FilterRAD, Filter>(m, "FilterRAD")
        .def(nb::init<bool, bool>());
}

void export_FilterSANN(nb::module_& m)
{
    nb::class_<FilterSANN, Filter>(m, "FilterSANN")
        .def(nb::init<bool>());
}
