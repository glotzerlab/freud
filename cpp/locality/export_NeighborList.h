        ...
        unsigned int num_points, nb_array<float> vectors, nb_array<float, nb::ndim<1>> weights)
{
    ...
    new (nlist) NeighborList(num_bonds, query_point_indices_data, num_query_points,
            point_indices_data, num_points, vectors_data, weights_data);
}
        ...
        const box::Box& box, const bool exclude_ii)
{
    ...
    new (nlist) NeighborList(points_data, query_points_data, box,
