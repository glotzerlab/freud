...
void ConstructFromArrays(NeighborList* nlist, nb_array<unsigned int, nanobind::ndim<1>> query_point_indices,
                         ...
                         nb_array<float, nanobind::shape<-1, 3>> vectors, nb_array<float, nanobind::ndim<1>> weights);

void ConstructAllPairs(NeighborList* nlist, nb_array<float, nanobind::shape<-1, 3>> points, nb_array<float, nanobind::shape<-1, 3>> query_points,
