                            ...
                            if (!(i == 0 && j == 0 && k == 0))
                                {
                                    ...
                                    m_image_list[n_images]
                                        = float(i) * latt_a + float(j) * latt_b + float(k) * latt_c;
                                    ++n_images;
                                }
                        }
                }
                            ...
                            if (overlap(m_aabb_query->m_aabb_tree.getNodeAABB(cur_node_idx), asphere))
                                {
                                    if (m_aabb_query->m_aabb_tree.isNodeLeaf(cur_node_idx))
                                        {
                                            while (cur_ref_p < m_aabb_query->m_aabb_tree.getNodeNumParticles(
                                                       cur_node_idx))
                                                {
                                                    // Neighbor j
                                                    const unsigned int j
                                                        = m_aabb_query->m_aabb_tree.getNodeParticleTag(
                                                            cur_node_idx, cur_ref_p);
                    ...
                    while (true)
                        {
                                    ...
                                    if (!m_exclude_ii || cur_p != np.ref_id)
                                        {
                                            ...
                                            if (m_search_extended)
                                                {
                                                    if (!m_all_distances.count(np.ref_id)
                            ...
                            if (m_current_neighbors.size() >= m_k)
                            ...
                            else if ((m_r_cur >= max_plane_distance) || (m_all_distances.size() >= m_k))
                                {
                                    ...
                                    for (std::map<unsigned int, float>::const_iterator it(
                                             m_all_distances.begin());
                            ...
                            else if (m_r_cur > min_plane_distance / 2)
                                {
                                    ...
                                    for (std::vector<NeighborPoint>::const_iterator it(
                                             m_current_neighbors.begin());
