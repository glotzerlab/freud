...
SymmetryCollection::SymmetryCollection(int maxL)
    : m_maxL(maxL), m_symmetric_orientation(quat<float>())
{
    if (m_maxL < 0)
        throw invalid_argument("maxL must be 0 or greater");
    size_t Mlm_size = (m_maxL + 1) * (m_maxL + 1);
    m_Mlm = shared_ptr<float>(new float[Mlm_size], default_delete<float[]>());

    memset((void*) m_Mlm.get(), 0, sizeof(float)*Mlm_size);

    m_Mlm_rotated = shared_ptr<float>(new float[Mlm_size], default_delete<float[]>());

    memset((void*) m_Mlm_rotated.get(), 0, sizeof(float)*Mlm_size);
...
void SymmetryCollection::rotate(const quat<float> &q) {
    ...
    for (int j = 2; j <= m_maxL; ++j) {
        ...
        for (int mprime = 1; mprime < j - 1; ++mprime) {
            ...
            // m = -mprime:
            WDTable[WDindex(j, mprime, -mprime)] = (
                ss * WDTable[WDindex(j - 1, mprime - 1, 1 - mprime)] -
            ...
            for (int m = -mprime + 1; m < mprime; ++m) {
                WDTable[WDindex(j, mprime, m)] = (
                    sH * WDTable[WDindex(j - 1, mprime - 1, m)] * Npp +
            ...
            // m = mprime:
            WDTable[WDindex(j, mprime, mprime)] = (
                cc * WDTable[WDindex(j - 1, mprime - 1, mprime - 1)] +
        ...
        // m = -mprime:
        WDTable[WDindex(j, j - 1, 1 - j)] = (
            ss * WDTable[WDindex(j - 1, j - 2, 2 - j)] -
            s  * WDTable[WDindex(j - 1, j - 1, 2 - j)] * Npn / Npp);

        // -mprime < m < mprime:
        for (int m = -j + 2; m < j - 1; ++m) {
            WDTable[WDindex(j, j - 1, m)] = (
                sH * WDTable[WDindex(j - 1, j - 2, m)] * Npp +
                c  * WDTable[WDindex(j - 1, j - 1, m)] * Npn) / sqrt((j + m) * (j - m));
        }

        // m = mprime:
        WDTable[WDindex(j, j - 1, j - 1)] = (
            cc * WDTable[WDindex(j - 1, j - 2, j - 2)] +
        ...
        // -mprime < m < mprime:
        for (int m = -j + 1; m < j; ++m) {
            WDTable[WDindex(j, j, m)] = \
                sH * WDTable[WDindex(j - 1, j - 1, m)] * Npp / sqrt((j + m) * (j - m));
        }
...
void SymmetryCollection::searchSymmetry(FoundSymmetry *foundsym) {
    ...
    for (unsigned int vi = 0; vi < vertexList->size(); ++vi) {
