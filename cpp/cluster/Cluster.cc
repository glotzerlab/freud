...
Cluster::Cluster(float r_cut) : m_r_cut(r_cut), m_num_particles(0), m_num_clusters(0)
{
    if (m_r_cut < 0.0f)
        throw invalid_argument("Cluster requires that r_cut must be non-negative.");
}
                      ...
                      freud::locality::QueryArgs qargs)
{
    ...
    freud::locality::loopOverNeighbors(
        ...
        [this, &dj](const freud::locality::NeighborBond& neighbor_bond) {
            ...
            if (neighbor_bond.distance < m_r_cut)
            {
