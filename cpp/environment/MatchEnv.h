...
public:
    ...
    **/
    ...
    //! in the simulation. If global is false, only compare the environments of neighboring particles.
    void cluster(const freud::locality::NeighborList* env_nlist, const freud::locality::NeighborList* nlist,
                 const vec3<float>* points, unsigned int Np, float threshold, bool hard_r = false,
    ...
    //! a given input motif, characterized by refPoints (of which there are numRef). Returns a vector of
    //! minimal RMSD values, one value per particle. NOTE that this does not guarantee an absolutely minimal
    //! RMSD. It doesn't figure out the optimal permutation of BOTH sets of vectors to minimize the RMSD.
    //! Rather, it just figures out the optimal permutation of the second set, the vector set used in the
    //! argument below. To fully solve this, we need to use the Hungarian algorithm or some other way of
    ...
    //! The bool registration controls whether we first use brute force registration to orient the second set
    //! of vectors such that it minimizes the RMSD between the two sets. If registration=True, then refPoints2
    ...
    // REGISTERED) environment e2. Return a std::pair of the rotation matrix that takes the vectors of e2 to
    // the vectors of e1 AND the mapping between the properly indexed vectors of the environments that gives
    // this RMSD. Populate the associated minimum RMSD. The bool registration controls whether we first use
    // brute force registration to orient the second set of vectors such that it minimizes the RMSD between
    // the two sets. NOTE that this does not guarantee an absolutely minimal RMSD. It doesn't figure out the
    // optimal permutation of BOTH sets of vectors to minimize the RMSD. Rather, it just figures out the
    // optimal permutation of the second set, the vector set used in the argument below. To fully solve this,
    // we need to use the Hungarian algorithm or some other way of solving the so-called assignment problem.
    std::pair<rotmat3<float>, BiMap<unsigned int, unsigned int>>
    minimizeRMSD(Environment& e1, Environment& e2, float& min_rmsd, bool registration);

    // Overload: Get the somewhat-optimal RMSD between the set of vectors refPoints1 and the set of vectors
    // refPoints2. Construct the environments accordingly, and utilize minimizeRMSD() as above. Arguments are
    // pointers to interface directly with python. Return a std::map (for ease of use) with the mapping
    // between vectors refPoints1 and refPoints2 that gives this RMSD. Populate the associated minimum RMSD.
    // The bool registration controls whether we first use brute force registration to orient the second set
    // of vectors such that it minimizes the RMSD between the two sets. NOTE that this does not guarantee an
    // absolutely minimal RMSD. It doesn't figure out the optimal permutation of BOTH sets of vectors to
    // minimize the RMSD. Rather, it just figures out the optimal permutation of the second set, the vector
    // set used in the argument below. To fully solve this, we need to use the Hungarian algorithm or some
