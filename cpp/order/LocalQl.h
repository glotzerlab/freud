 ...
 *
 * The computeAve functions compute a variation of the Steinhardt Ql order parameter that attempts to account
 * for a second neighbor shell. For a particle i, we calculate the average Q_l by summing the spherical
...
public:
    ...
    virtual void compute(const locality::NeighborList* nlist, const vec3<float>* points, unsigned int Np);

    //! Compute the order parameter averaged over the second neighbor shell
    virtual void computeAve(const locality::NeighborList* nlist, const vec3<float>* points, unsigned int Np);
