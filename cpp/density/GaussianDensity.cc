                ...
                for (int k = bin_z - bin_cut_z; k <= bin_z + bin_cut_z; k++)
                    {
                        ...
                        for (int j = bin_y - bin_cut_y; j <= bin_y + bin_cut_y; j++)
                            {
                                float dy = float((grid_size_y * j + grid_size_y / 2.0f) - points[idx].y
                                                 - ly / 2.0f);
                                        ...
                                        if (rsqrt < m_rcut)
                                            {
                                                ...
                                                float x_gaussian = A
                                                    * exp((-1.0f) * (delta.x * delta.x) / (2.0f * sigmasq));
                                                float y_gaussian = A
                                                    * exp((-1.0f) * (delta.y * delta.y) / (2.0f * sigmasq));
                                                float z_gaussian = A
                                                    * exp((-1.0f) * (delta.z * delta.z) / (2.0f * sigmasq));
