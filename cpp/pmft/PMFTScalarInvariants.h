...
#ifndef _PMFTScalarInvariants_H__
#define _PMFTScalarInvariants_H__

/*! \internal
    \file PMFTScalarInvariants.h
...
*/
class PMFTScalarInvariants
    {
    public:
        //! Constructor
        PMFTScalarInvariants(float max_R, float max_S, float max_U, float max_RW, float max_RV, float max_WT,
        ...
        */
        ...
        //! Get a reference to the S array
        std::shared_ptr<float> get_S()
            {
            return m_S_array;
            }

        //! Get a reference to the U array
        std::shared_ptr<float> get_U()
            {
            return m_U_array;
            }
        //! Get a reference to the r*W array
        std::shared_ptr<float> get_RW()
            {
            return m_RW_array;
            }

        //! Get a reference to the r*V array
        std::shared_ptr<float> get_RV()
            {
            return m_RV_array;
            }
        //! Get a reference to the r*V array
        std::shared_ptr<float> get_WT()
            {
            return m_WT_array;
            }
        ...
        unsigned int getNBins_S()
            {
            return m_n_bins_S;
            }
        unsigned int getNBinsU()
            {
            return m_n_bins_U;
            }

        unsigned int getNBins_RW()
            {
            return m_n_bins_RW;
            }

        unsigned int getNBinsRV()
            {
            return m_n_bins_RV;
            }

        unsigned int getNBins_WT()
            {
            return m_n_bins_WT;
            }
    ...
    private:
        ...
        float m_max_S;                     //!< Maximum S at which to compute pcf
        float m_max_U;                     //!< Maximum U at which to compute pcf
        float m_max_RW;                     //!< Maximum r*W at which to compute pcf
        float m_max_RV;                     //!< Maximum r*V at which to compute pcf
        float m_max_WT;                     //!< Maximum W*T at which to compute pcf
        float m_dR;                       //!< Step size for R in the computation
        float m_d_S;                       //!< Step size for S in the computation
        float m_d_U;                       //!< Step size for U in the computation
        float m_d_RW;                       //!< Step size for r*W in the computation
        float m_d_RV;                       //!< Step size for r*V in the computation
        float m_d_WT;                       //!< Step size for W*T in the computation
        locality::LinkCell* m_lc;          //!< LinkCell to bin particles for the computation
        unsigned int m_n_bins_R;             //!< Number of R bins to compute pcf over
        unsigned int m_n_bins_S;             //!< Number of S bins to compute pcf over
        unsigned int m_n_bins_U;             //!< Number of U bins to compute pcf over
        unsigned int m_n_bins_RW;             //!< Number of r*W bins to compute pcf over
        unsigned int m_n_bins_RV;             //!< Number of r*V bins to compute pcf over
        unsigned int m_n_bins_WT;             //!< Number of W*T bins to compute pcf over
        ...
        std::shared_ptr<float> m_S_array;           //!< array of S values that the pcf is computed at
        std::shared_ptr<float> m_U_array;           //!< array of U values that the pcf is computed at
        std::shared_ptr<float> m_RW_array;           //!< array of r*W values that the pcf is computed at
        std::shared_ptr<float> m_RV_array;           //!< array of r*V values that the pcf is computed at
        std::shared_ptr<float> m_WT_array;           //!< array of W*T values that the pcf is computed at
        tbb::enumerable_thread_specific<unsigned int *> m_local_bin_counts;
    };

}; }; // end namespace freud::pmft

#endif // _PMFTScalarInvariants_H__
