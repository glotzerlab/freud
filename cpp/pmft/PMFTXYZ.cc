      ...
      m_n_faces(0), m_shiftvec(shiftvec)
{
    ...
    m_bin_counts = std::shared_ptr<unsigned int>(new unsigned int[m_n_x * m_n_y * m_n_z],
...
void PMFTXYZ::reducePCF()
{
    ...
    float norm_factor
        = (float) 1.0 / ((float) this->m_frame_counter * (float) this->m_n_ref * (float) m_n_faces);
    // normalize pcf_array
    parallel_for(blocked_range<size_t>(0, m_n_x * m_n_y * m_n_z), [=](const blocked_range<size_t>& r) {
        for (size_t i = r.begin(); i != r.end(); i++)
            {
                m_pcf_array.get()[i]
