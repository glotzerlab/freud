    ...
    : PMFT(), m_x_max(x_max), m_y_max(y_max), m_t_max(2.0 * M_PI), m_n_x(n_x), m_n_y(n_y), m_n_t(n_t)
{
    ...
    m_bin_counts = std::shared_ptr<unsigned int>(new unsigned int[m_n_x * m_n_y * m_n_t],
    ...
    m_local_bin_counts.updateSize(m_n_x * m_n_y * m_n_t);
}
...
void PMFTXYT::reducePCF()
{
    ...
    // normalize pcf_array
    parallel_for(blocked_range<size_t>(0, m_n_x * m_n_y * m_n_t), [=](const blocked_range<size_t>& r) {
        for (size_t i = r.begin(); i != r.end(); i++)
        {
