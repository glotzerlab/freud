                  ...
                  nb_array<float, nb::shape<-1, 3>> out)
{
    ...
    vec3<float>* vecs_data = reinterpret_cast<vec3<float>*>(vecs.data());
    vec3<float>* out_data = reinterpret_cast<vec3<float>*>(out.data());
               ...
               nb_array<int, nb::shape<-1, 3>> images)
{
    ...
    vec3<float>* vecs_data = reinterpret_cast<vec3<float>*>(vecs.data());
    vec3<int>* images_data = reinterpret_cast<vec3<int>*>(images.data());
                      ...
                      nb_array<float, nb::ndim<1>> distances)
{
    ...
    vec3<float>* query_points_data = reinterpret_cast<vec3<float>*>(query_points.data());
    const unsigned int n_points = points.shape(0);
    vec3<float>* points_data = reinterpret_cast<vec3<float>*>(points.data());
    float* distances_data = reinterpret_cast<float*>(distances.data());
...
void contains(std::shared_ptr<Box> box, nb_array<float> points, nb_array<bool, nb::ndim<1>> contains_mask)
{
    const unsigned int n_points = points.shape(0);
    vec3<float>* points_data = reinterpret_cast<vec3<float>*>(points.data());
    bool* contains_mask_data = reinterpret_cast<bool*>(contains_mask.data());
