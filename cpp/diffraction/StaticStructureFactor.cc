...
StaticStructureFactor::StaticStructureFactor(unsigned int bins, float k_max, float k_min)
{
    ...
    const float delta = (k_max - k_min) / float(bins);
    const float target_k_min = k_min - delta * 0.5;
    const float target_k_max = k_max + delta * 0.5;
    const auto axes = util::Axes {std::make_shared<util::RegularAxis>(bins + 1, target_k_min, target_k_max)};
