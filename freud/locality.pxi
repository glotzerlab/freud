...
cdef class NeighborList:
    ...
    @property
    def index_i(self):
        """The reference point indices from the last set of points we
    ...
    @property
    def index_j(self):
        """The target point indices from the last set of points we
        were evaluated with. This array is read-only to prevent
        breakage of find_first_index."""
        cdef np.npy_intp size[2]
        size[0] = self.thisptr.getNumBonds()
    ...
    @property
    def weights(self):
        """The per-bond weights from the last set of points we were
        evaluated with"""
    ...
    @property
    def segments(self):
        """A *segment array*, which is an array of length `N_ref`
...
cdef class LinkCell:
    ...
    def getNumCells(self):
        """Get the the number of cells in this box.
...
cdef class NearestNeighbors:
    ...
    :param float scale: multiplier by which to automatically increase rmax
                        value by if requested number of neighbors is not found.
                        Only utilized if strict_cut is False. Scale must be
                        greater than 1.
    :param bool strict_cut: whether to use a strict rmax or allow for automatic
                            expansion
    ...
    def getNRef(self):
        """Get the the number of particles this object found neighbors of.
