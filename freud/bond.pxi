...
cdef class BondingAnalysis:
    ...
    def initialize(self, frame_0):
        ...
        """
        frame_0 = freud.common.convert_array(frame_0, 2, dtype=np.uint32, contiguous=True,
                                             dim_message="frame_0 must be a 2 dimensional array")
        if (frame_0.shape[0] != self.num_particles):
            raise ValueError(
                "the 1st dimension must match num_particles: {}".format(self.num_particles))
        if (frame_0.shape[1] != self.num_bonds):
            raise ValueError(
                "the 2nd dimension must match num_bonds: {}".format(self.num_bonds))
    ...
    def compute(self, frame_0, frame_1):
        ...
        frame_0 = freud.common.convert_array(frame_0, 2, dtype=np.uint32, contiguous=True,
                                             dim_message="frame_0 must be a 2 dimensional array")
        frame_1 = freud.common.convert_array(frame_1, 2, dtype=np.uint32, contiguous=True,
...
cdef class BondingR12:
    ...
    def compute(self, box, ref_points, ref_orientations, points, orientations, nlist=None):
        ...
        with nogil:
            self.thisptr.compute(l_box, nlist_ptr, < vec3[float]*> l_ref_points.data, < float*> l_ref_orientations.data, n_ref,
...
cdef class BondingXYZ:
    ...
    def compute(self, box, ref_points, ref_orientations, points, orientations, nlist=None):
        ...
        cdef _box.Box l_box = _box.Box(box.getLx(), box.getLy(), box.getLz(), box.getTiltFactorXY(),
                                       box.getTiltFactorXZ(), box.getTiltFactorYZ(), box.is2D())
        with nogil:
            self.thisptr.compute(l_box, nlist_ptr, < vec3[float]*> l_ref_points.data, < quat[float]*> l_ref_orientations.data, n_ref,
