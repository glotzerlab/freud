...
namespace wrap {
        void compute(const std::shared_ptr<Cluster> & self, const std::shared_ptr<locality::NeighborQuery> nq, const std::shared_ptr<locality::NeighborList> nlist,
                const locality::QueryArgs& qargs, nb_array<unsigned int, nanobind::shape<-1>> keys)
{
        unsigned int* keys_data = nullptr;
        if(!keys.is_none()){
            keys_data = reinterpret_cast<unsigned int*>(keys.data());
        }
...
namespace detail {
    void export_Cluster(nb::module_& module)
    {
        nanobind::class_<Cluster>(module, "Cluster")
            .def(nb::init<>())
            .def("compute", &wrap::compute, nanobind::arg("nq"), nanobind::arg("nlist"), nanobind::arg("qargs"), nanobind::arg("keys_data").none())
