...
cdef class RAD(_Compute):
    r"""Computes coordination shell using the RAD algorithm.

    The Relative Angular Distance (RAD) :cite:`Higham2016` algorithm defines
    the coordination shell for a query particle as follows:

    A particle's coordination shell includes all particles that are not blocked
    by any other particle and not further away than a blocked particle.

    "Blocking" is determined by checking the distances between the query
    particle and its two potential neighbors, as well as the angle formed by
    the three particles.
    """
