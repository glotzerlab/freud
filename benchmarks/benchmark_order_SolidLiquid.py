...
class BenchmarkOrderSolidLiquid(Benchmark):
    ...
    def bench_setup(self, N):
        ...
        self.points = np.asarray(np.random.uniform(-self.L/2, self.L/2,
                                                   (N, 3)),
                                 dtype=np.float32)
        self.sl = freud.order.SolidLiquid(self.sph_l, self.Qthreshold,
                                          self.Sthreshold)

    def bench_run(self, N):
        self.sl.compute((self.box, self.points),
...
def run():
    ...
    return run_benchmarks(name, Ns, number, BenchmarkOrderSolidLiquid,
                          **kwargs)
