...
#
# The benchmark class sets up and runs benchmarks of given functions and/or methods and provides convenience routines
# for one time setup and multiple run invocations. Benchmarks may be run with profiling enabled, with simple timing,
# or with speedup numbers on multiple cores.
#
# It provides an N argument to set the benchmark size. Benchmarks that don't use an N can simply ignore it (and the user
# can set None). When N is present, timing results are normalized by N.
#
# Like the unit test module, it is designed to be an overridden class. Users must override run() to run their benchmark.
